...
def berkopec_single_term(k, m, K, M):
    ...
    NOTE: This function divides two very large numbers, which introduces numerical errors of the order of 10-16. These errors can pile up when summing the terms, which can change significantly the result of the algorithm for computing the hypergeometric tail inverse with Berkopec's forumla when delta is smaller than the numerical error. A work around is to normalize at the end only. See for example 'hypergeometric_tail_inverse'.
...
def berkopec_hypergeometric_tail_inverse(k, m, delta, M, start='below'):
    ...
    Both algorithms are in worst case O(M-m) and in Ω(1) in best case. If k is small, the bisection algorithm of 'hypergeometric_tail_inverse' is probably faster.
...
def logberkopec_hypergeometric_tail_inverse(k, m, log_delta, M, start='below'):
    ...
    See the doc of the function 'hypergeometric_tail_inverse' for more info.
    ...
    This algorithm is the same as in 'hypergeometric_tail_inverse', but in logarithmic form to limit memory usage and to increase efficiency using optimized algorithms to compute the binomial coefficients. We use the identity
...
def naive_hypergeometric_tail_inverse(k, m, delta, M, start='below'):
    """
    ...
    if start == 'above':
        K = k
        hyp_cdf = hypergeometric_tail(k, m, K, M)
