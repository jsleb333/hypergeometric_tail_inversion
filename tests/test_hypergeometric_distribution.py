...
def test_berkopec_formula_equals_left_tail():
    ...
    # assert np.isclose(berkopec, hypergeometric_left_tail(k, m, K, M), atol=10-35, rtol=10e-20)
...
def test_naive_hypergeometric_left_tail_inverse_above_is_inverse():
    k, m, K, M = 5, 13, 16, 30
    assert naive_hypergeometric_left_tail_inverse(k, m, hypergeometric_left_tail(k,m,K,M), M, start='above') == K
    assert naive_hypergeometric_left_tail_inverse(k, m, hypergeometric_left_tail(k,m,K,M)-0.001, M, start='above') == K+1
    assert naive_hypergeometric_left_tail_inverse(k, m, hypergeometric_left_tail(k,m,K,M)+0.001, M, start='above') == K

    for delta in [0.05, 0.1, 0.25]:
        assert hypergeometric_left_tail(k, m, naive_hypergeometric_left_tail_inverse(k,m,delta,M, start='above'), M) <= delta
