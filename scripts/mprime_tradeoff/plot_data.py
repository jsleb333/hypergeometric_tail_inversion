...
def plot_comp_delta(m, k, d, deltas):
    ...
    plot.add_plot(mprimes, bounds, color, label=f"\\footnotesize $\\delta={delta}$", line_join='round', label_anchor='-120')
...
def plot_comp_m(ms, k, d, delta):
    ...
    for m, color in zip(ms, palette[1:]):
        ...
        plot.add_plot(mprimes_plot, bounds_plot, color, label=f"\\footnotesize ${m=}$", line_join='round')
...
if __name__ == "__main__":
    ...
    # plot = plot_comp_k(ms[1], ks, ds[1], deltas[2])
    # plot = plot_comp_d(ms[1], ks[0], ds, deltas[2])
    plot = plot_comp_delta(ms[1], ks[0], ds[1], deltas)
    # plot = plot_comp_m(ms, ks[0], ds[1], deltas[2])
