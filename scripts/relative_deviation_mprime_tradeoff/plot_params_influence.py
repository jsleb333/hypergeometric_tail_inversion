...
def plot_comp_k(m, ks, d, delta=0.05):
    ...
    for k, color in zip(ks, palette[1:]):
        ...
        idx = list(range(0, 2000, 1)) + list(range(2000, 20_000, 500)) + [19_999]
        mprimes = df['mprime']
        bounds = df['bound']
        mprimes_plot = [mprimes[i] for i in idx]
        bounds_plot = [bounds[i] for i in idx]

        plot.add_plot(mprimes_plot, bounds_plot, color, label=f"\\footnotesize $k={k}$", line_join='round')

        min_idx = np.argmin(bounds)
        min_bound = bounds[min_idx]
        min_mprime = mprimes[min_idx]
        plot.add_plot([min_mprime], [min_bound], color, 'only marks', mark_size='1.25pt', label=f'\\scriptsize ${min_mprime}$', label_anchor='south')
...
def plot_comp_d(m, k, ds, delta=0.05):
    ...
    for d, color in zip(ds, palette[1:]):
        datafile = f'./data/mprime_tradeoff-{m=}_{k=}_{d=}_{delta=}.csv'
        df = pd.read_csv(datafile, sep=',', header=0)
        idx = list(range(0, 2000)) + list(range(2000, 20_000, 500)) + [19_999]
        mprimes = df['mprime']
        bounds = df['bound']
        mprimes_plot = [mprimes[i] for i in idx]
        bounds_plot = [bounds[i] for i in idx]
        plot.add_plot(mprimes_plot, bounds_plot, color, label=f"\\footnotesize ${d=}$", line_join='round')
...
def plot_comp_delta(m, k, d, deltas):
    ...
    plot.add_plot(mprimes_plot, bounds_plot, color, label=f"\\footnotesize $\\delta={delta}$", line_join='round', label_anchor='-155')
    ...
    min_bound = bounds[min_idx]
    min_mprime = mprimes[min_idx]
    plot.add_plot([min_mprime], [min_bound], color, 'only marks', mark_size='1.25pt', label=f'\\scriptsize ${min_mprime}$', label_anchor='north')
...
def plot_comp_m(ms, k, d, delta):
    ...
    for m, color in zip(ms, palette[::2]):
        ...
        mprimes_plot = [mprimes[i] for i in idx]
        bounds_plot = [bounds[i] for i in idx]
        plot.add_plot(mprimes_plot, bounds_plot, color, label=f"\\scriptsize ${m=}$", line_join='round', label_anchor='180')
...
if __name__ == "__main__":
    os.chdir('./scripts/mprime_tradeoff_relative_deviation/')
    ...
    filename = f'mprime_tradeoff_reldev_comp_k'
    plot = plot_comp_k(200, ks, 10, 0.05)

    # filename = f'mprime_tradeoff_reldev_comp_d'
    # plot = plot_comp_d(200, 0, ds, 0.05)

    # filename = f'mprime_tradeoff_reldev_comp_delta'
    # plot = plot_comp_delta(200, 0, 10, deltas)

    # filename = f'mprime_tradeoff_reldev_comp_m'
